...
sortedDF$teden <- factor(sortedDF$teden)
sortedDF$mesec <- factor(sortedDF$mesec)
...
```{r}
#library(plot3D)
scatter3D(sortedDF$smer_vetra, sortedDF$hitrost_vetra, sortedDF$temp_zraka, clab = c("Temperatura zraka", "smer_vetra, hitrost_vetra"))
```
...
```{r Napoved po naivnem Bayesu}
i = 1
predictedBayes <- list()
for (nb in nb_list) {
  tempDF <- filter(sortedDF, mesec == i+1)
  predictedBayes[[i]] <- predict(nb, tempDF, type = "class")
  i = i + 1
}
```
...
for (nb in nb_list) {
  tempDF <- filter(sortedDF, mesec == i+1)
  predictedMat[[i]] <- predict(nb, tempDF, type = "raw")
  i = i + 1
}
col.order <- c("ZELONIZKA", "NIZKA", "SREDNJA", "VISOKA", "ZELOVISOKA")
head(predictedMat[[1]][,col.order])
...
for (knn in knn_list_kInNN) {
  predictedMatKInKNN[[i]] <- predict(knn, tempDF, type = "prob")
  bar <- sum((observedMat[[1]] - predictedMatKInKNN[[i]]) ^ 2) / nrow(predictedMatKInKNN[[i]])
  cat("Barier's score za k =", i, "je", bar, "\n")
  i = i + 1
}
```
...
for (rf in rf_list) {
  tic(i)
  tempDF <- filter(sortedDF, mesec == i+1)
  predictedRF[[i]] <- predict(rf, tempDF, type = "class")
  i = i + 1
  toc()
}
toc()
...
for (rf in rf_list) {
  tic(i)
  tempDF <- filter(sortedDF, mesec == i+1)
  predictedMatRF[[i]] <- predict(rf, tempDF, type = "prob")
  toc()
  i = i + 1
}
col.order <- c("ZELONIZKA", "NIZKA", "SREDNJA", "VISOKA", "ZELOVISOKA")
