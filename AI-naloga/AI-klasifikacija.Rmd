...
Podatke za regresijo se vzame iz že urejenih podatkov "sortedDF" brez "norm_poraba" in se jih faktorizira. Zaradi prevelike količine podatkov se preveri za vsak mesec posebej(npr. zgoraj za januar).
...
summary(dataForRegression)
```
Množica podatkov se zmanjša na 9 atributov, ki najbolj pripomorejo k napovedovanju atributa "poraba".
...
```
Z vsemi podatki, torej 30% testna množica, 70% učna se dobi zgornje rešitve. Prikazano je regresijsko drevo,
prva vrnjena številka je srednja absolutna napaka, druga pa relativna srednja absolutna napaka.
...
rmae(test$poraba, predicted, mean(train$poraba))
```
Naključni gozd vrne zaenkrat najboljše rezultate in sicer s srednjo absolutno napako 27,7 in relativno srednjo
absolutno napako 0,18.
